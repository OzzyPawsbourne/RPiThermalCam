...
def rpi_temp(data,point,palette_switch=True,circle_flag=True,text_flag=True):
    ...
    # Change Color Map
    data = cv2.applyColorMap(data,generate_colour_map(palette_switch))

    if circle_flag:
        data = put_circle(data,point)
    if text_flag:
        data = put_text(data,temp_str)
        
    return data

def put_circle(data,point):
    radius = 4
    color = (0,255,0)
    thickness = 1
    return cv2.circle(data, point, radius, color, thickness)

def put_text(data,temp_str):
...
def generate_colour_map(palette_switch):
    """
